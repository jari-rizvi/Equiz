        ...
        viewModelScope.launch {
            ...
            if (networkHelper.isNetworkConnected()) {
                try {
                    ...
                    mainRepository.getBannersDashboard(country/*true*/).let {
                        if (it.isSuccessful) {
...
//                            _getBannerResponse2.postValue(Resource.error(it.message(), null))
    ...
    private val _getExchangeRateResponse = MutableLiveData<Resource<ExchangeRateData>>()
    val getExchangeRateResponse: LiveData<Resource<ExchangeRateData>>
        get() = _getExchangeRateResponse

    fun getExchangeRates(
        params: JsonObject,
    ) {
        viewModelScope.launch {
            _getExchangeRateResponse.postValue(Resource.loading(null))
            if (networkHelper.isNetworkConnected()) {
                try {
                    Timber.tag("87878787887").d("starta")

                    mainRepository.exchangeRate(params).let {
                        if (it.isSuccessful) {
                            _getExchangeRateResponse.postValue(Resource.success(it.body()!!))
                            Timber.tag("87878787887").d(it.body()!!.toString())
                        } else if (it.code() == 401) {
                            _getExchangeRateResponse.postValue(Resource.unAuth("", null))
                        } else if (it.code() == 500 || it.code() == 409 || it.code() == 502 || it.code() == 404 || it.code() == 400) {
                            Timber.tag("87878787887").d("secoonnddd")

//                            _getExchangeRateResponse.postValue(Resource.error(it.message(), null))
                            val jsonObj = JSONObject(it.errorBody()!!.charStream().readText())
                            _getExchangeRateResponse.postValue(Resource.error(jsonObj.getString("message")))
                        } else {
                            _getExchangeRateResponse.postValue(
                                Resource.error(
                                    "Some thing went wrong",
                                    null
                                )
                    ...
                    _getExchangeRateResponse.postValue(Resource.error("${e.message}", null))
                }
            } else _getExchangeRateResponse.postValue(Resource.error("No internet connection", null))
        }
    }
