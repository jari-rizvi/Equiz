...
//                            _orderDetailResponse.postValue(Resource.error(it.message(), null))
   ...
   private val _cancelProductResponse = MutableLiveData<Resource<OrderDetailData>>()
    val cancelProductResponse: LiveData<Resource<OrderDetailData>>
        get() = _cancelProductResponse


    fun cancelProduct(id: String, params: JsonObject) {
        viewModelScope.launch {
            _cancelProductResponse.postValue(Resource.loading(null))
            if (networkHelper.isNetworkConnected()) {
                try {
                    Timber.tag("87878787887").d( "starta")

                    mainRepository.cancelProductOrder(id,params).let {
                        if (it.isSuccessful) {
                            _cancelProductResponse.postValue(Resource.success(it.body()!!))
                            Timber.tag("87878787887").d( it.body()!!.toString())
                        }  else if (it.code() == 500 || it.code() == 409 || it.code() == 502 || it.code() == 404 || it.code() == 400) {
                            Timber.tag("87878787887").d( "secoonnddd")

//                            _cancelProductResponse.postValue(Resource.error(it.message(), null))
                            val jsonObj = JSONObject(it.errorBody()!!.charStream().readText())
                            _cancelProductResponse.postValue(Resource.error(jsonObj.getString("message")))
                        } else {
                            _cancelProductResponse.postValue(
                                Resource.error(
                                    "Some thing went wrong",
                                    null
                                )
                    ...
                    _cancelProductResponse.postValue(Resource.error("${e.message}", null))
                }
            } else _cancelProductResponse.postValue(Resource.error("No internet connection", null))
        }
    }
